...
export default function CheckoutPage(){
  ...
  useEffect(()=>{
    ...
    loadTossPayments(clientKey)
      .then(async(toss)=>{
        ...
        await paymentWidget.setAmount({currency:"KRW",value:order.amount});
        await Promise.all([
          paymentWidget.renderPaymentMethods({selector:"#payment-method",variantKey:"DEFAULT"}),
      ...
      .catch((error)=>{
        ...
        setMessage(`결제 위젯을 불러오지 못했습니다: ${reason}`);
      });
  },[order]);
  ...
  return <div className="min-h-screen bg-zinc-50 px-4 py-10"><div className="mx-auto max-w-2xl rounded-3xl bg-white p-5 shadow-sm sm:p-8"><p className="text-xs font-extrabold tracking-[3px] text-[#00C471]">CHECKOUT</p><h1 className="mt-3 text-2xl font-extrabold">안전한 결제</h1>{order&&<div className="mt-6 flex justify-between rounded-2xl bg-zinc-50 p-5"><span>{order.orderName}</span><strong>{order.amount.toLocaleString()}원</strong></div>}<div id="payment-method" className="mt-6"/><div id="agreement"/><button onClick={pay} disabled={!order||!!message} className="mt-5 w-full rounded-xl bg-[#00C471] py-4 font-bold text-white disabled:opacity-40">{order?`${order.amount.toLocaleString()}원 결제하기`:"준비 중"}</button>{message&&<p className="mt-4 text-center text-sm text-zinc-500">{message}</p>}</div></div>;
}
