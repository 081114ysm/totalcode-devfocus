...
- `P2_코드/frontend`: Next.js 16 프런트엔드
- `P2_코드/backend`: Express 5 API
- `P2_문서`: API, ERD, P1~P3 운영 문서
...
```bash
cp P2_코드/backend/.env.example P2_코드/backend/.env
cp P2_코드/frontend/.env.example P2_코드/frontend/.env.local
...
`P2_문서/p3/runbook.md`를 참고합니다.

## 검증

```bash
cd P2_코드/frontend && npm ci && npm run lint && npm run build
